...
Address.prototype = {
  ...
  generate: function() {
    ...
    setTimeout(function() {
      ...
      var pubkeys = this.masterNodes.map(function(node) {
        var keyPair = node.hd_node.derive(this.index, node.compressed).keyPair;
        ...
        if (!node.compressed) {
          // Uncompress key if necessary.
          return (new Bitcoin.ECPair(null, keyPair.Q, {
      ...
      this.redeemScript = Bitcoin.script.multisig.output.encode(constants.M, this.pubkeys); // M of N;

      // Generate the multisig address for this redeem script.
      var scriptPubKey = Bitcoin.script.scriptHash.output.encode(Bitcoin.crypto.hash160(this.redeemScript));
      this.address = Bitcoin.address.fromOutputScript(scriptPubKey);
  ...
  setUnspentOutputs: function(response) {
    ...
    this.unspentOutputs = response.map(function(output) {
      return {
        ...
        script: new Buffer(output.scriptPubKey, 'hex')
      }
    });
  ...
  getKeyPair: function(node) {
    return node.derive(this.index).keyPair;
  },
  comparePubKeys: function(a, b) {
    if (a.toString('hex') < b.toString('hex')) return -1;
    if (b.toString('hex') < a.toString('hex')) return 1;
    return 0;
  }
}
