...
var Transaction = function(options) {
  ...
  this.tb.enableBitcoinCash(true);
  this.inputAddresses = [];
}
Transaction.prototype = {
  ...
  addInputs: function() {
    ...
    $.each(this.addresses, function(i, address) {
      ...
      $.each(address.unspentOutputs, function(i, output) {
        that.tb.addInput(output.hash, output.index, 0xffffffff, output.script);
  ...
  signInputs: function() {
    ...
    var sigHashType = Bitcoin.Transaction.SIGHASH_ALL | Bitcoin.Transaction.SIGHASH_BITCOINCASHBIP143;
    ...
    $.each(this.inputAddresses, function(address_index, address) {
      ...
      $.each(address.pubkeys, function(pubkey_index, pubkey) {
        $.each(that.hdWallets, function(hd_index, hd) {
          ...
          that.tb.sign(address_index, privateKeyPair, address.redeemScript, sigHashType, address.balance);
        })
      })
    })
  }
